...
// Mapping des intérêts aux catégories d'activités pour le modèle de recommandation
...
export const getRecommendedActivities = (interests: string[], limit: number = 7): Activity[] => {
  ...
  if (filteredActivities.length < limit) {
    ...
    filteredActivities = [
      ...filteredActivities,
      ...remainingActivities.sort(() => 0.5 - Math.random()).slice(0, limit - filteredActivities.length)
    ];
  }
